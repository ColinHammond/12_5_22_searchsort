...
def main():
    student_data = get_data("student.txt")
    student_data.sort(key=get_key)
    #above sorts student_data
...
average = find_average(get_data("student.txt"))
print(f"the average gpa was {average}")

#recursive_sum = recursive_sum_of_credits()get_data("student.txt")
